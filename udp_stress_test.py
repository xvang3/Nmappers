...
def start_servers(clients, servers, udp_filename):
    first_client = clients[0]
    for server in servers:
        server.cmd(f'iperf -s -u -t 30 -b {TRAFFIC_INTENSITY} >> {udp_filename} 2>&1 &')
        latency = measure_latency(first_client, server)
        print("Latency for UDP (Server " + server.name + "): " + latency)

def start_clients(clients, servers, traffic_intensity, udp_filename):
    for client in clients:
        selected_server = random_server(servers)
        client.cmd('iperf -c -u ' + selected_server.IP() + ' -b ' + traffic_intensity + ' >> {udp_filename} 2>&1 &')
        latency = measure_latency(client, selected_server)
        print("Latency for UDP (Client " + client.name + " to Server " + selected_server.name + "): " + latency)
...
def main():
    ...
    udp_filename = f'iPerf_udp_stress_test_results_{timestamp}.txt'

    # Setup the network
    net, switches, clients, servers = setup_network()

    base_latency = measure_latency(clients[0], servers[0])
    print("Baseline Latency for UDP: " + base_latency)

    # Start the servers
    start_servers(clients, servers, udp_filename)

    # Start the clients
    start_clients(clients, servers, TRAFFIC_INTENSITY, udp_filename)
